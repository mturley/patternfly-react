...
test('should call onChangeUsername callback', () => {
  ...
  const view = shallow(
    <LoginForm onChangeUsername={mockFn} rememberMeLabel="Login Form" />
  );
...
test('should call onChangePassword callback', () => {
  const mockFn = jest.fn();
  const view = shallow(
    <LoginForm onChangePassword={mockFn} rememberMeLabel="Login Form" />
  );
...
test('should call onChangeRememberMe callback', () => {
  const mockFn = jest.fn();
  const view = shallow(
    <LoginForm onChangeRememberMe={mockFn} rememberMeLabel="Login Form" />
  );
