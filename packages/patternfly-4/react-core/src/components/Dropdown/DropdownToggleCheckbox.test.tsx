...
test('controlled', () => {
  const view = shallow(
    <DropdownToggleCheckbox isChecked id="check" aria-label="check" />
  );
...
test('uncontrolled', () => {
  const view = shallow(
    <DropdownToggleCheckbox id="check" aria-label="check" />
  );
  expect(view).toMatchSnapshot();
});
...
test('3rd state', () => {
  const view = shallow(
    <DropdownToggleCheckbox id="check" isChecked={null} aria-label="check" />
  );
  expect(view).toMatchSnapshot();
});
...
test('checkbox passes value and event to onChange handler', () => {
  ...
  const view = mount(
    <DropdownToggleCheckbox id="check" {...props} aria-label="check" />
  );
