...
const components = {
  wrapper: componentProps => (
    <div className="theme-pf-c-content" {...componentProps} />
  ),
  code: LiveEdit,
  pre: React.Fragment,
  p: componentProps => <Text component="p" {...componentProps} />,
  blockquote: componentProps => (
    <Text component="blockquote" {...componentProps} />
  ),
...
for (let i = 1; i <= 6; i++) {
  components[`h${i}`] = componentProps => (
    <Text component={`h${i}`} {...componentProps} />
  );
}
    ...
    default:
      return (
        <React.Fragment>
          ...
          <a href={`../../components/${state}`}>production-level component</a>{' '}
          and will be removed in a future release. Use the production-ready
          version of this feature instead."
        </React.Fragment>
      );
  }
};
    ...
    <SidebarLayout>
      <PageSection>
        ...
        {data.mdx.frontmatter.stage && (
          <Alert
            variant="warning"
            title={getWarning(data.mdx.frontmatter.stage)}
          />
        )}
      ...
      {cssPrefix && (
        <PageSection>
          <CSSVars
            caption={
              <p>
                CSS Variables starting with <strong>--{cssPrefix}</strong>{' '}
                from&nbsp;
                <a
...
export const pageQuery = graphql`
  query GetComponent(
    ...
    props: allComponentMetadata(
      filter: { name: { in: $propComponents }, path: { regex: $pathRegex } }
    ) {
      nodes {
        name
        props {
