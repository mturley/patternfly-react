describe('Nav Test', () => {
  ...
  it('Verify Simple Nav', () => {
    cy.get('#nav-primary-simple .pf-c-nav__link').each(
      (simpleNavLink: JQuery<HTMLAnchorElement>, index: number) => {
  ...
  it('Verify Default Nav', () => {
    ...
    cy.get('#default-link3').then((defaultLink3: JQuery<HTMLAnchorElement>) => {
      ...
      cy.url().should(
        'eq',
        'http://localhost:3000/nav-demo-nav-link#default-link3'
      );
      cy.get('#nav-primary-default .pf-c-nav__link').each(
        (defaultNavLink: JQuery<HTMLAnchorElement>, index: number) => {
          const isCurrent = defaultNavLink.hasClass('pf-m-current');
    ...
    cy.get('#default-no-navigate-link').then(
      (noNavigateLink: JQuery<HTMLAnchorElement>) => {
  ...
  it('Verify Expandable Nav', () => {
    ...
    cy.get(
      '#nav-primary-expandable .pf-c-nav__link[data-component="pf-nav-expandable"]'
    ).each((expandableGroup: JQuery<HTMLAnchorElement>, index: number) => {
  ...
  it('Verify Horizontal Nav', () => {
    cy.get(
      '#nav-primary-horizontal .pf-c-nav__horizontal-list .pf-c-nav__link'
    ).each((horizontalLink: JQuery<HTMLAnchorElement>, index: number) => {
      const isCurrent = horizontalLink.hasClass('pf-m-current');
      expect(isCurrent).to.be.equal(index === 0);
    });
    cy.get('#nav-primary-horizontal #horizontal-link2').then(
      (horizontalLink2: JQuery<HTMLAnchorElement>) => {
