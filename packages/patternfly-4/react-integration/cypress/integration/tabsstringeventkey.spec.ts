describe('Tab Demo Test', () => {
  it('Navigate to demo section', () => {
    ...
    cy.url().should(
      'eq',
      'http://localhost:3000/tab-string-event-key-demo-nav-link'
    );
  });

  it('Verify tabs with string event keys, and tab navigation', () => {
    cy.get('.pf-c-tabs__button').each(
      (demoButton: JQuery<HTMLButtonElement>, index: number) => {
        const currentItem: number = index + 1;
        expect(demoButton.text()).to.equal(`Tab item ${currentItem}`);
        cy.wrap(demoButton).click();
        cy.get(`#demoTab${currentItem}Section`).then(
          (demoSection: JQuery<HTMLElement>) => {
