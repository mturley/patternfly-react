describe('Login Page Demo Test', () => {
  ...
  it('Verify initial state', () => {
    ...
    cy.get('#options-menu-single-option-example')
      ...
      .should('exist');
    cy.get('#options-menu-single-option-example.pf-m-expanded').should(
      'not.exist'
    );

    cy.get('#options-menu-modified-example.pf-m-top.pf-m-align-right').should(
      'exist'
    );
  ...
  it('Verify event handlers fire correctly', () => {
    cy.get('#options-menu-single-option-example')
      ...
      .then((toggleButton: JQuery<HTMLButtonElement>) => {
        ...
        cy.get('#singleOption2').then(
          (secondMenuItem: JQuery<HTMLButtonElement>) => {
            cy.wrap(secondMenuItem).click();
            cy.get('#singleOption2')
              .find('.pf-c-options-menu__menu-item-icon')
              .then(
                secondMenuItemIcon => expect(secondMenuItemIcon).to.be.visible
              );
            cy.get('#singleOption1')
              .find('.pf-c-options-menu__menu-item-icon')
              .then(
                firstMenuItemIcon => expect(firstMenuItemIcon).to.be.hidden
              );
          }
        );
        cy.wrap(toggleButton).click();
    ...
    cy.get('#options-menu-modified-example')
      ...
      .then((toggleButton: JQuery<HTMLButtonElement>) => {
        ...
        cy.wrap(toggleButton).click();
        cy.get('#options-menu-modified-example.pf-m-expanded').should(
          'not.exist'
        );
      });
  });
