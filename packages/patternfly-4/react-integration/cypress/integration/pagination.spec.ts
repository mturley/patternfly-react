describe('Pagination Demo Test', () => {
  ...
  it('should be disabled when flag is present', () => {
    cy.get('#pagination-options-menu-disabled')
      ...
      .then(toggleText =>
        expect(toggleText).to.have.text('1 - 20 of 523 items')
      );
    cy.get('#pagination-options-menu-disabled')
    ...
    cy.get('#pagination-options-menu-disabled')
      ...
      .then(button => expect(button).to.be.disabled);
    cy.get(
      '#pagination-options-menu-disabled > .pf-c-options-menu > .pf-c-dropdown'
    )
      .find('button')
      .then(button => expect(button).to.be.disabled);
  });
  ...
  it('Verify initial state', () => {
    ...
    cy.get(
      '#pagination-options-menu-bottom.pf-c-pagination.pf-m-footer'
    ).should('exist');
  ...
  it('Verify event handlers fire correctly', () => {
    cy.get('#pagination-options-menu-top')
      ...
      .then((toggleButton: JQuery<HTMLButtonElement>) => {
        ...
        cy.get('button[data-action="per-page-10"]').then(
          (firstMenuItem: JQuery<HTMLButtonElement>) => {
            cy.wrap(firstMenuItem).click();
            cy.get('.pf-c-dropdown.pf-m-expanded').should('not.exist');
            cy.get('#pagination-options-menu-top')
              .find('.pf-c-options-menu__toggle-text')
              .then(toggleText =>
                expect(toggleText).to.have.text('1 - 10 of 523 items')
              );
            cy.get('#pagination-options-menu-top')
              .find('.pf-c-pagination__nav-page-select')
              .then(navPageSelect =>
                expect(navPageSelect).to.have.text('of 53 pages')
              );
    ...
    cy.get('#pagination-options-menu-top')
      ...
      .then((button: JQuery<HTMLButtonElement>) => {
        ...
        cy.get('#pagination-options-menu-top')
          .find('.pf-c-options-menu__toggle-text')
          .then(toggleText =>
            expect(toggleText).to.have.text('11 - 20 of 523 items')
          );
        cy.get('#pagination-options-menu-top')
          .find('.pf-c-pagination__nav-page-select input')
