...
export const getBulletLegendY = ({
  ...
  switch (legendPosition) {
    ...
    case 'right': {
      ...
      const legendPadding = (legendData: any[]) =>
        legendData && legendData.length > 0 ? 17 : 0;
      return (
        (chartSize.height - legendDimensions.height) / 2 +
        legendPadding(legendData)
      );
    }
    default:
      return dy;
  ...
  switch (legendPosition) {
    case 'bottom':
      return width > legendDimensions.width - textSizeWorkAround
        ? Math.round(
            (width - (legendDimensions.width - textSizeWorkAround)) / 2
          ) + dx
        : dx;
    case 'bottom-left':
      return left + dx;
...
export const getChartLegendY = ({
  ...
  switch (legendPosition) {
    ...
    case 'right': {
      ...
      const legendPadding = (legendData: any[]) =>
        legendData && legendData.length > 0 ? 2 : 0;
