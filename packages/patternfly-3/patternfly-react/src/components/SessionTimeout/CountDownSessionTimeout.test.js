...
test('CountDownSessionTimeout init timeLeft > displayBefore', () => {
  ...
  let component = mount(
    <CountDownSessionTimeout timeLeft={10} displayBefore={3} sessionTime={sessionTime} />
  );
...
test('CountDownSessionTimeout init timeLeft < displayBefore', () => {
  ...
  const sessionTime = 15;
  let component = mount(
    <CountDownSessionTimeout timeLeft={5} displayBefore={10} sessionTime={sessionTime} />
  );
  expect(component.find(MessageDialog).prop('show')).toBe(true);
...
test('CountDownSessionTimeout clicking logout terminates session', () => {
  ...
  const component = mount(
    <CountDownSessionTimeout timeLeft={4} displayBefore={4} sessionTime={sessionTime} />
  );
  expect(component.find(MessageDialog).prop('show')).toBe(true);
