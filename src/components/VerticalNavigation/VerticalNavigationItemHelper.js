...
// TODO -- this helper used to be the item class itself.
//      -- it has a ton of stuff that can probably be lifted up to the primary/secondary/tertiary components.

class BaseVerticalNavigationItemHelper extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      hoverTimer: null,
      ...
      (children &&
        ...
        React.Children.toArray(children).filter(
          child =>
            ...
            child.type.displayName.includes('VerticalNavigation') &&
            child.type.displayName.includes('Item')
        )) ||
      (subItems &&
        subItems.length > 0 &&
        subItems.map(childItem => (
          <VerticalNavigationItemHelper
            item={childItem}
            key={childItem.title}
          />
        )));
...
BaseVerticalNavigationItemHelper.propTypes = {
...
BaseVerticalNavigationItemHelper.defaultProps = {
  title: '',
  selectedMobileDepth: null
};

const VerticalNavigationItemHelper = controlled(
  controlledStateTypes,
  defaultControlledState
)(consumeNavContext(BaseVerticalNavigationItemHelper));

VerticalNavigationItemHelper.displayName = 'VerticalNavigationItemHelper';

export default VerticalNavigationItemHelper;
