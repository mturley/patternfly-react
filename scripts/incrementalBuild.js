...
async function getInvalidPackages() {
  const packages = (await new Project(__dirname).getPackages())
    ...
    .filter(p =>
      isPf3 ? p.location.indexOf('patternfly-3') > 0 || commonPackages.indexOf(p.name) >= 0 : true
    ) // Based off argv
    .filter(p =>
      isPf4 ? p.location.indexOf('patternfly-4') > 0 || commonPackages.indexOf(p.name) >= 0 : true
    ); // Based off argv
