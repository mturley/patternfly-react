...
async function getCorePackages() {
  const packages = (await new Project(__dirname).getPackages()).filter(p => {
    if (
      p.dependencies &&
      Object.keys(p.dependencies).includes('@patternfly/patternfly')
    ) {
      return true;
    }
    if (
      p.devDependencies &&
      Object.keys(p.devDependencies).includes('@patternfly/patternfly')
    ) {
      return true;
    }
    return false;
...
getCorePackages().then(packages =>
  packages.forEach(package => {
    ...
    if (
      ...
      json.devDependencies['@patternfly/patternfly']
    ) {
